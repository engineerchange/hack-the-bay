...
# use tyr_2019, not t1 below tyr(2019) %>% write_csv(")
...
tplot <- function(t1){
cols = c("1-5"="lightyellow","6-8"="yellow","9-12"="orange",">12"="red")
gg <- ggplot() + theme_bw() + geom_sf(data=huc %>% mutate(HUC12=case_when(
    substr(HUC12,1,1) == "0" ~ substr(HUC12,2,nchar(HUC12)),
    TRUE ~ HUC12
  )) %>% right_join(t1 %>% mutate(HUC12=as.character(HUC12)),by="HUC12"),aes(fill=col)) +
...
library(patchwork)
tyr(2016);tfilters(t1);tplot(t1) + ggtitle("2016") -> gg1; gg1 <- gg1
tyr(2017);tfilters(t1);tplot(t1) + ggtitle("2017") -> gg2; gg2 <- gg2
tyr(2018);tfilters(t1);tplot(t1) + ggtitle("2018") -> gg3; gg3 <- gg3
tyr(2019);tfilters(t1);tplot(t1) + ggtitle("2019") -> gg4
gg1 + gg2 + gg3 + (gg4) + plot_annotation(title='CBP') +
  plot_layout(nrow = 1,guides='collect') & theme(legend.position='right')
...
gg1 + gg2 + gg3 + gg4 + plot_annotation(title='CMC',caption = "Parameters: Light Yellow < 5, Yellow = 6-8, Orange = 9-12, Red = >12") +
  plot_layout(nrow = 1) & theme(plot.caption=element_text(size=16))
